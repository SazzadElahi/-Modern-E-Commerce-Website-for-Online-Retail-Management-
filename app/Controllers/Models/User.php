...
class User
{
    ...
    {
        $s = $db->prepare("SELECT id,name,email,phone,address FROM users WHERE id=?");
        $s->bind_param("i", $id);
        $s->execute();
    ...
    public static function create(mysqli $db, string $name, string $email, string $password, string $phone, string $address): int
    {
        ...
        $s = $db->prepare("INSERT INTO users(name,email,password_hash,phone,address) VALUES(?,?,?,?,?)");
        $s->bind_param("sssss", $name, $email, $hash, $phone, $address);
        $s->execute();
        return $db->insert_id;
    }
    public static function emailTaken(mysqli $db, string $email, int $except = 0): bool
    {
    ...
    public static function update(mysqli $db, int $id, string $name, string $email, string $phone, string $address): bool
    {
        $s = $db->prepare("UPDATE users SET name=?,email=?,phone=?,address=? WHERE id=?");
        $s->bind_param("ssssi", $name, $email, $phone, $address, $id);
        return $s->execute();
    }
    public static function all(mysqli $db): mysqli_result
    {
        return $db->query("SELECT id,name,email,phone,created_at FROM users ORDER BY id DESC");
    }
